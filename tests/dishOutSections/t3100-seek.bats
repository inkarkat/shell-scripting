...
@test "seek to random existing counts and print the successors" {
    run -0 dishOutSections --seek 2 "${BATS_TEST_DIRNAME}/input.txt"
...
EOF
    ...
    run -0 dishOutSections --seek 4 "${BATS_TEST_DIRNAME}/input.txt"
    assert_output ''

    run -0 dishOutSections --seek 1 "${BATS_TEST_DIRNAME}/input.txt"
